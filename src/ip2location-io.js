...
import punycode from "npm:punycode@2.3.0";
...
export class Configuration {
  ...
  // Return the IP2Location.io API key
  getApiKey() {
    return this.#apiKey;
  }
...
export class DomainWhois {
  ...
  getDomainName(fullURL) {
    ...
    try {
      ...
      let host = this.getPunycode(myURL.hostname);

      for (let i = 0; i < tlds.length; i++) {
        if (host.endsWith(tlds[i])) {
          let tmp = host.substring(0, host.lastIndexOf(tlds[i]));

          let domain = host.substring(tmp.lastIndexOf(".") + 1);
          return domain;
        }
      }
    } catch (error) {
  ...
  getDomainExtension(str) {
    let domain = this.getDomainName(str);
